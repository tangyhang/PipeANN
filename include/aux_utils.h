...
namespace diskann {
  ...
  template<typename T>
  DISKANN_DLLEXPORT int build_merged_vamana_index(std::string base_file, diskann::Metric _compareMetric,
  ...
  DISKANN_DLLEXPORT bool build_disk_index(const char *dataFilePath, const char *indexFilePath,
                                          const char *indexBuildParameters, diskann::Metric _compareMetric,
                                          bool single_file_index, const char *tag_file = nullptr);
