...
int main(int argc, char **argv) {
  if (argc != 4) {
    diskann::cout << argv[0] << " <ground_truth_bin> <our_results_bin>  <r> " << std::endl;
    return -1;
  }
  ...
  if (points_num_gs != points_num_or) {
    diskann::cout << "Error. Number of queries mismatch in ground truth and our results" << std::endl;
    return -1;
  }
  ...
  if ((dim_or < recall_at) || (recall_at > dim_gs)) {
    diskann::cout << "ground truth has size " << dim_gs << "; our set has " << dim_or << " points. Asking for recall "
                  << recall_at << std::endl;
    return -1;
  }
  diskann::cout << "Calculating recall@" << recall_at << std::endl;
  float recall_val = (float) diskann::calculate_recall((_u32) points_num, gold_std, gs_dist, (_u32) dim_gs, our_results,
