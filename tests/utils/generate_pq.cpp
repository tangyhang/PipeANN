...
bool generate_pq(const std::string &data_path, const std::string &index_prefix_path, const size_t num_pq_centers,
                 const size_t num_pq_chunks, const float sampling_rate) {
  ...
  diskann::cout << "For computing pivots, loaded sample data of size " << train_size << std::endl;

  generate_pq_pivots(train_data, train_size, (_u32) train_dim, (_u32) num_pq_centers, (_u32) num_pq_chunks,
                     KMEANS_ITERS_FOR_PQ, pq_pivots_path);
  generate_pq_data_from_pivots<T>(data_path, (_u32) num_pq_centers, (_u32) num_pq_chunks, pq_pivots_path,
...
int main(int argc, char **argv) {
  if (argc != 6) {
    ...
    if (std::string(argv[1]) == std::string("float"))
      generate_pq<float>(data_path, index_prefix_path, num_pq_centers, num_pq_chunks, sampling_rate);
    else if (std::string(argv[1]) == std::string("int8"))
      generate_pq<int8_t>(data_path, index_prefix_path, num_pq_centers, num_pq_chunks, sampling_rate);
    else if (std::string(argv[1]) == std::string("uint8"))
      generate_pq<uint8_t>(data_path, index_prefix_path, num_pq_centers, num_pq_chunks, sampling_rate);
    else
