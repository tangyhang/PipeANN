...
float distanceCosine_I(const int8_t *a, const int8_t *b, size_t size) {
  ...
  std::cout << "aMag: " << aMag << " bMag: " << bMag << " product: " << scalarProduct << std::endl;
...
uint8_t *getData(const char *infileName, uint8_t dataTypeSizeInBytes, int32_t &count, int32_t &dimension) {
  ...
  diskann::cout << infileName << ": count: " << count << " dimensions: " << dimension << std::endl;
...
void concat_files(int argc, char **argv) {
  if (argc < 5) {
    diskann::cout << "Mode 1 requires 2 input files and 1 output file as argument." << std::endl
  ...
  if (dimension1 != dimension2) {
    diskann::cout << "Error! Cannot combine vectors of differing dimensions (" << dimension1 << "," << dimension2 << ")"
                  << std::endl;
    return;
  }
...
void compareDistanceComputationsInt() {
...

  {
    ...
    if (abs(dist1 - dist2) > epsilon) {
      std::cout << "compareDistanceComputationsInt(): L2 Test failed. AVX dist: " << dist1 << " normal dist: " << dist2
                << " difference > " << epsilon << std::endl;
    } else {
  ...
  }
  {
    ...
    if (abs(dist1 - dist2) > epsilon) {
      std::cout << "compareDistanceComputationsInt(): Cosine Test failed. AVX dist: " << dist1
                << " normal dist: " << dist2 << " difference > " << epsilon << std::endl;
    } else {
      std::cout << "Two scores are the same. " << std::endl;
    }
  }
...
void compareDistanceComputationsFloat() {
...

  {
    ...
    if (abs(dist1 - dist2) > epsilon) {
      std::cout << "compareDistanceComputationsFloat(): L2 Test failed. AVX dist: " << dist1
                << " normal dist: " << dist2 << " difference > " << epsilon << std::endl;
    } else {
      std::cout << "Two scores are the same." << std::endl;
    }
  }
  {
    ...
    if (abs(dist1 - dist2) > epsilon) {
      std::cout << "compareDistanceComputationsFloat(): Cosine Test failed. "
                   "AVX dist: "
                << dist1 << " normal dist: " << dist2 << " difference > " << epsilon << std::endl;
    } else {
      std::cout << "Two scores are the same." << std::endl;
    }
  }
...
int main(int argc, char **argv) {
  if (argc < 2) {
    diskann::cout << std::string("Usage: ") << argv[0] << " <mode> [arguments]" << std::endl
                  << "Modes: 1 for file concat. Args <file1> <file2> <outfile>" << std::endl
                  << "       2 for test unique_ptr assignment. No args." << std::endl
                  << "      3 for comparing distance computations (AVX and "
                     "normal). No args."
                  << std::endl
                  << "      4 for testing our streambuf() implemntation. No args" << std::endl
                  << "      5 for testing membuf implementation. Args <outfile>" << std::endl
                  << "      6 for testing substring implementation. Args <s1> <s2>" << std::endl
                  << "      7 for testing OpenMP thread pool implementation. No args." << std::endl;
    return -1;
  }
  ...
  switch (mode) {
    ...
    case 7:
      ...
      break;
    default:
      diskann::cout << "Don't know what to do with mode parameter: " << argv[1] << std::endl;
  }
}
