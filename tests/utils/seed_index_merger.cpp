...
void dump_to_disk(const T *all_pts, const uint64_t ndims, const std::string &filename,
  ...
  for (uint64_t i = 0; i < tags.size(); i++) {
    ...
    *(new_tags + i) = tags[i];
    memcpy(new_data + (i * ndims), all_pts + (tags[i] * ndims), ndims * sizeof(float));
  }
  //  tag_writer.close();
...
template<typename T>
void run(const uint32_t base_count, const uint32_t num_mem_indices, const uint32_t delete_count,
         const uint32_t incr_count, const uint32_t num_cycles, const std::string &in_file,
  ...
  while (count++ < num_cycles) {
    ...
    float active_tags_sampling_rate =
        (float) ((std::min)((1.0 * delete_count) / (1.0 * ((double) active_tags.size())), 1.0));
    ...
    float inactive_tags_sampling_rate =
        (float) ((std::min)((1.0 * incr_count) / (1.0 * ((double) inactive_tags.size())), 1.0));

    for (auto iter = inactive_tags.begin(); iter != inactive_tags.end(); iter++) {
    ...
    diskann::cout << "Merge program will insert " << insert_vec.size() << " points and delete  " << delete_vec.size()
    ...
    const uint64_t mem_count = ROUND_UP(insert_vec.size(), num_mem_indices) / num_mem_indices;
    ...
    for (uint64_t i = 0; i < num_mem_indices; i++) {
      ...
      diskann::cout << "Index #" << i + 1 << " : choosing " << new_start - cur_start << " points\n";
      ith_tags.insert(ith_tags.end(), insert_vec.begin() + cur_start, insert_vec.begin() + new_start);
      cur_start = new_start;
    }
    ...
    for (uint64_t i = 0; i < num_mem_indices; i++) {
      ...
      dump_to_disk(all_points, ndims, out_prefix + "_cycle_" + std::to_string(count) + "_mem_" + std::to_string(i + 1),
                   mem_tags[i]);
    }

    // re-shuffle tags to get delete list
    //  std::shuffle(tags.begin(), tags.end(), rng);
    std::ofstream deleted_tags_writer(deleted_tags_file + "_cycle_" + std::to_string(count));
...
int main(int argc, char **argv) {
  if (argc != 10) {
    diskann::cout << "Correct usage: " << argv[0] << " <type[int8/uint8/float]> <base_count> <num_mem_indices>"
  ...
  if (index_type == std::string("float")) {
    run<float>(base_count, num_mem_indices, delete_count, incr_count, num_cycles, in_file, out_prefix,
               deleted_tags_file);
  } else if (index_type == std::string("uint8")) {
    run<uint8_t>(base_count, num_mem_indices, delete_count, incr_count, num_cycles, in_file, out_prefix,
                 deleted_tags_file);
  } else if (index_type == std::string("int8")) {
    run<int8_t>(base_count, num_mem_indices, delete_count, incr_count, num_cycles, in_file, out_prefix,
                deleted_tags_file);
  } else {
